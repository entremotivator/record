...
# ============================================================================
# OPENAI TRANSCRIPTION FUNCTIONS
# ============================================================================
...
# ============================================================================
def generate_show_notes(episode_title, transcription, api_key):
    """Generate professional show notes from transcription using OpenAI"""
    try:
        client = OpenAI(api_key=api_key)
        
        prompt = f"""Based on this podcast episode transcription, generate professional show notes in markdown format.
...
4. Timestamps for major topics (if possible)
    ...
    if st.session_state.podcast_series:
        ...
        for series in st.session_state.podcast_series:
            ...
            with st.expander(f"📻 {series.get('name', 'Untitled')} ({len(series_episodes)} episodes)"):
                ...
                with col3:
                    if st.button("✏️ Edit", key=f"edit_series_{series.get('id')}"):
                        st.session_state.current_series = series
                        st.rerun()
                    
                    if st.button("🗑️ Delete", key=f"delete_series_{series.get('id')}"):
        ...
        for ep in recent_episodes:
            ...
            with col1:
                st.write(f"**{ep.get('title', 'Untitled')}**")
                st.caption(f"Episode #{ep.get('episode_number', 'N/A')} • {ep.get('status', 'draft').upper()}")
        ...
        st.info("👈 Create your first podcast series in the 'Series' tab to get started!")

# ============================================================================
# TAB 2: PODCAST SERIES MANAGEMENT
# ============================================================================
with tab2:
    ...
    with subtab1:
        ...
        with col1:
            series_name = st.text_input("Series Name", placeholder="My Awesome Podcast")
            series_author = st.text_input("Author Name", placeholder="Your Name")
        ...
        with col2:
            series_language = st.selectbox("Language", ["English", "Spanish", "French", "German", "Other"])
            series_email = st.text_input("Contact Email", placeholder="podcast@example.com")
            series_website = st.text_input("Website (optional)", placeholder="https://example.com")
        
        series_description = st.text_area("Series Description", height=150, placeholder="Describe your podcast...")
        
        if st.button("✅ Create Series", use_container_width=True):
        ...
        if st.session_state.podcast_series:
            for series in st.session_state.podcast_series:
                with st.expander(f"📻 {series.get('name', 'Untitled')}", expanded=False):
                    ...
                    with col2:
                        if st.button("✏️ Edit", key=f"edit_s_{series.get('id')}"):
                            st.session_state.current_series = series
                            st.rerun()
                        
                        if st.button("🗑️ Delete", key=f"del_s_{series.get('id')}"):
                            st.session_state.podcast_series.remove(series)
                            st.rerun()
        else:
...
with tab3:
    ...
    if not st.session_state.podcast_series:
        ...
        with subtab1:
            ...
            episode_title = st.text_input("Episode Title", placeholder="Episode Title")
            episode_description = st.text_area("Episode Description", height=100)
            
            col1, col2 = st.columns(2)
            
            with col1:
                season_number = st.number_input("Season Number", min_value=1, value=1, step=1)
                guest_names = st.text_input("Guest Names (comma-separated)", placeholder="Guest 1, Guest 2")
            
            with col2:
                publish_date = st.date_input("Publish Date")
                episode_status = st.selectbox("Status", ["draft", "scheduled", "published"])
            ...
            if uploaded_file and st.session_state.openai_api_key:
                ...
                with col2:
                    if st.button("📝 Generate Show Notes", use_container_width=True):
                ...
                if st.button("💾 Save Episode", use_container_width=True):
        ...
        with subtab2:
            ...
            if st.session_state.episodes:
                search_term = st.text_input("🔍 Search episodes", placeholder="Search by title...")
                ...
                for ep in filtered_episodes:
                    with st.expander(f"📺 {ep.get('title')} (Ep #{ep.get('episode_number')})", expanded=False):
                        ...
                        with col3:
                            if st.button("✏️ Edit", key=f"edit_ep_{ep.get('id')}"):
                                st.session_state.current_episode = ep
                                st.rerun()
                            
                            if st.button("🗑️ Delete", key=f"del_ep_{ep.get('id')}"):
        ...
        with subtab3:
            ...
            with col2:
                ...
                new_status = st.selectbox("New Status", ["draft", "scheduled", "published"])
                
                if st.button("Update All Episodes", use_container_width=True):
...
with tab4:
    ...
    with subtab1:
        ...
        with col1:
            ...
            guest_email = st.text_input("Email", placeholder="guest@example.com")
            guest_website = st.text_input("Website (optional)", placeholder="https://example.com")
        
        with col2:
            guest_social = st.text_input("Social Media (optional)", placeholder="@twitter_handle")
            guest_photo_url = st.text_input("Photo URL (optional)", placeholder="https://example.com/photo.jpg")
        
        guest_bio = st.text_area("Bio", height=150, placeholder="Tell us about this guest...")
    ...
    with subtab2:
        ...
        if st.session_state.guests:
            search_guest = st.text_input("🔍 Search guests", placeholder="Search by name...")
            ...
            for guest in filtered_guests:
                ...
                with col2:
                    if st.button("🗑️ Delete", key=f"del_guest_{guest.get('id')}"):
    ...
    if st.session_state.podcast_series:
        ...
        with col1:
            ...
            for platform in platforms:
                ...
                with col_check:
                    st.checkbox(f"📻 {platform}", key=f"platform_{platform}")
                with col_status:
                    st.caption("⏳ Pending")
        ...
        with col2:
            ...
            feed_url = st.text_input("RSS Feed URL (auto-generated)", placeholder="https://example.com/feed.xml", disabled=True)
            
            st.markdown("#### Publishing Options")
            
            if st.button("🚀 Publish All Episodes", use_container_width=True):
                for ep in st.session_state.episodes:
                    ep['status'] = 'published'
            ...
            if st.button("🔗 Generate Podcast Feed", use_container_width=True):
...
with tab7:
    ...
    subtab1, subtab2, subtab3 = st.tabs(["General", "API Keys", "Export & Backup"])
    ...
    with subtab1:
        ...
        with col1:
            st.checkbox("Email notifications for new episodes")
            st.checkbox("Remind me to publish scheduled episodes")
        
        with col2:
            st.checkbox("Weekly analytics summary")
            st.checkbox("Notify on guest confirmations")
    ...
    with subtab2:
        ...
        if st.button("🔄 Reset Google Drive"):
            st.session_state.gdrive_authenticated = False
